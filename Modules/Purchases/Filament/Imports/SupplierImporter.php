...
namespace Modules\Purchases\Filament\Imports;
        ...
        return [
            ImportColumn::make('type')
                ...
                ->castStateUsing(fn (?string $state): ?string => match (mb_strtolower((string) $state)) {
                    'individual', 'pf', 'pessoa física', 'pessoa fisica' => SupplierType::Individual->value,
                    'company', 'pj', 'pessoa jurídica', 'pessoa juridica' => SupplierType::Company->value,
                    default => $state,
                })
                ->rules(['required', Rule::in([SupplierType::Individual->value, SupplierType::Company->value])]),
            ...
            ImportColumn::make('document')
                ...
                ->rules(fn (array $data, ?Model $record): array => [
                    ...
                    'string',
                    'max:14',
                    Rule::unique('suppliers', 'document')->ignore($record),
                    ($data['type'] ?? null) === SupplierType::Company->value ? new ValidCnpj : new ValidCpf,
                ]),
